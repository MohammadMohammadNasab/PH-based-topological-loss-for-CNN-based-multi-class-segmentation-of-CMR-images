...
image = torch.load(r'C:\Users\Mohammad\Desktop\ML_Project\topological-losses\data\image.pt')
label = torch.load(r'C:\Users\Mohammad\Desktop\ML_Project\topological-losses\data\label.pt')
...
plt.subplot(1, 2, 2)
plt.imshow(label.cpu().numpy().squeeze(), cmap='gray')
plt.title(f'Label (shape: {label.shape})')
plt.axis('off')
